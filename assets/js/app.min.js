...
Array.prototype.chunk=function(chunkSize){for(var R=[],i=0;i<this.length;i+=chunkSize)R.push(this.slice(i,i+chunkSize));return R};
...
"use strict";angular.module("angular-google-analytics",[]).provider("Analytics",function(){var accountId,domainName,removeRegExp,experimentId,created=!1,trackRoutes=!0,trackPrefix="",analyticsJS=!1,pageEvent="$routeChangeSuccess",cookieConfig="auto",ecommerce=!1,enhancedLinkAttribution=!1,ignoreFirstPageLoad=!1;this._logs=[],this.setAccount=function(id){return accountId=id,!0},this.trackPages=function(doTrack){return trackRoutes=doTrack,!0},this.trackPrefix=function(prefix){return trackPrefix=prefix,!0},this.setDomainName=function(domain){return domainName=domain,!0},this.useAnalytics=function(val){return analyticsJS=!!val,!0},this.useEnhancedLinkAttribution=function(val){return enhancedLinkAttribution=!!val,!0},this.setPageEvent=function(name){return pageEvent=name,!0},this.setCookieConfig=function(config){return cookieConfig=config,!0},this.useECommerce=function(val){return ecommerce=!!val,!0},this.setRemoveRegExp=function(regex){return regex instanceof RegExp?(removeRegExp=regex,!0):!1},this.setExperimentId=function(id){return experimentId=id,!0},this.ignoreFirstPageLoad=function(val){ignoreFirstPageLoad=!!val},this.$get=["$document","$rootScope","$location","$window",function($document,$rootScope,$location,$window){function _createScriptTag(){accountId&&($window._gaq=[],$window._gaq.push(["_setAccount",accountId]),enhancedLinkAttribution&&$window._gaq.push(["_require","inpage_linkid","//www.google-analytics.com/plugins/ga/inpage_linkid.js"]),trackRoutes&&!ignoreFirstPageLoad&&(removeRegExp?$window._gaq.push(["_trackPageview",getUrl()]):$window._gaq.push(["_trackPageview"])),domainName&&$window._gaq.push(["_setDomainName",domainName]),function(){var document=$document[0],ga=document.createElement("script");ga.type="text/javascript",ga.async=!0,ga.src=("https:"===document.location.protocol?"https://ssl":"http://www")+".google-analytics.com/ga.js";var s=document.getElementsByTagName("script")[0];s.parentNode.insertBefore(ga,s)}(),created=!0)}function _createAnalyticsScriptTag(){if(!accountId)return console.warn("No account id set for Analytics.js");if(function(i,s,o,g,r,a,m){i.GoogleAnalyticsObject=r,i[r]=i[r]||function(){(i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date,a=s.createElement(o),m=s.getElementsByTagName(o)[0],a.async=1,a.src=g,m.parentNode.insertBefore(a,m)}(window,document,"script","//www.google-analytics.com/analytics.js","ga"),$window.ga("create",accountId,cookieConfig),trackRoutes&&!ignoreFirstPageLoad&&$window.ga("send","pageview",getUrl()),$window.ga&&(ecommerce&&$window.ga("require","ecommerce","ecommerce.js"),enhancedLinkAttribution&&$window.ga("require","linkid","linkid.js"),experimentId)){var expScript=document.createElement("script"),s=document.getElementsByTagName("script")[0];expScript.src="//www.google-analytics.com/cx/api.js?experiment="+experimentId,s.parentNode.insertBefore(expScript,s)}}var getUrl=function(){var url=$location.path();return removeRegExp?url.replace(removeRegExp,""):url};this._log=function(){this._logs.push(arguments)},this._trackPage=function(url,title){title=title?title:$document[0].title,trackRoutes&&!analyticsJS&&$window._gaq?($window._gaq.push(["_set","title",title]),$window._gaq.push(["_trackPageview",trackPrefix+url]),this._log("_trackPageview",arguments)):trackRoutes&&analyticsJS&&$window.ga&&($window.ga("send","pageview",{page:trackPrefix+url,title:title}),this._log("pageview",arguments))},this._trackEvent=function(category,action,label,value){!analyticsJS&&$window._gaq?($window._gaq.push(["_trackEvent",category,action,label,value]),this._log("trackEvent",arguments)):$window.ga&&($window.ga("send","event",category,action,label,value),this._log("event",arguments))},this._addTrans=function(transactionId,affiliation,total,tax,shipping,city,state,country,currency){!analyticsJS&&$window._gaq?($window._gaq.push(["_addTrans",transactionId,affiliation,total,tax,shipping,city,state,country]),this._log("_addTrans",arguments)):$window.ga&&(ecommerce?($window.ga("ecommerce:addTransaction",{id:transactionId,affiliation:affiliation,revenue:total,tax:tax,shipping:shipping,currency:currency||"USD"}),this._log("ecommerce:addTransaction",arguments)):console.warn("ecommerce no set. Use AnalyticsProvider.setECommerce(true);"))},this._addItem=function(transactionId,sku,name,category,price,quantity){!analyticsJS&&$window._gaq?($window._gaq.push(["_addItem",transactionId,sku,name,category,price,quantity]),this._log("_addItem",arguments)):$window.ga&&($window.ga("ecommerce:addItem",{id:transactionId,name:name,sku:sku,category:category,price:price,quantity:quantity}),this._log("ecommerce:addItem",arguments))},this._trackTrans=function(){!analyticsJS&&$window._gaq?($window._gaq.push(["_trackTrans"]),this._log("_trackTrans",arguments)):$window.ga&&($window.ga("ecommerce:send"),this._log("ecommerce:send",arguments))},this._clearTrans=function(){$window.ga&&($window.ga("ecommerce:clear"),this._log("ecommerce:clear",arguments))},this._send=function(obj){$window.ga&&($window.ga("send",obj),this._log("send",obj))},analyticsJS?_createAnalyticsScriptTag():_createScriptTag();var me=this;return trackRoutes&&$rootScope.$on(pageEvent,function(){me._trackPage(getUrl())}),{_logs:me._logs,cookieConfig:cookieConfig,ecommerce:ecommerce,enhancedLinkAttribution:enhancedLinkAttribution,getUrl:getUrl,experimentId:experimentId,ignoreFirstPageLoad:ignoreFirstPageLoad,trackPage:function(url,title){me._trackPage(url,title)},trackEvent:function(category,action,label,value){me._trackEvent(category,action,label,value)},addTrans:function(transactionId,affiliation,total,tax,shipping,city,state,country,currency){me._addTrans(transactionId,affiliation,total,tax,shipping,city,state,country,currency)},addItem:function(transactionId,sku,name,category,price,quantity){me._addItem(transactionId,sku,name,category,price,quantity)},trackTrans:function(){me._trackTrans()},clearTrans:function(){me._clearTrans()},send:function(obj){me._send(obj)}}}]});
/*
...
angular.module("napp.menu",[]).directive("menu",["$location",function(location){return{restrict:"AE",transclude:!0,replace:!0,scope:{title:"@"},controller:function($scope){$scope.items=[],this.addItem=function(match,element){$scope.items.push({element:element,match:match});var reg=new RegExp(match);reg.test(location.path())&&angular.element(element).addClass("active")},$scope.$on("$routeChangeStart",function(){var route=location.path();for(var item in $scope.items){item=$scope.items[item];var reg=new RegExp(item.match);reg.test(route)?angular.element(item.element).addClass("active"):angular.element(item.element).removeClass("active")}})},templateUrl:"assets/html/menu/menu.html"}}]).directive("menuItemParent",function(){return{require:"^menu",restrict:"AE",replace:!0,transclude:!0,scope:{match:"@",title:"@"},link:function(scope,element,attrs,menuCtrl){menuCtrl.addItem(scope.match,element)},templateUrl:"assets/html/menu/menu-item-parent.html"}}).directive("menuItem",function(){return{require:"^menu",restrict:"AE",replace:!0,transclude:!0,scope:{match:"@",route:"@",title:"@"},link:function(scope,element,attrs,menuCtrl){menuCtrl.addItem(scope.match,element)},templateUrl:"assets/html/menu/menu-item.html"}});
var mod;mod=angular.module("infinite-scroll",[]),mod.value("THROTTLE_MILLISECONDS",null),mod.directive("infiniteScroll",["$rootScope","$window","$timeout","THROTTLE_MILLISECONDS",function($rootScope,$window,$timeout,THROTTLE_MILLISECONDS){return{scope:{infiniteScroll:"&",infiniteScrollContainer:"=",infiniteScrollDistance:"=",infiniteScrollDisabled:"=",infiniteScrollUseDocumentBottom:"="},link:function(scope,elem,attrs){var changeContainer,checkWhenEnabled,container,handleInfiniteScrollContainer,handleInfiniteScrollDisabled,handleInfiniteScrollDistance,handleInfiniteScrollUseDocumentBottom,handler,immediateCheck,scrollDistance,scrollEnabled,throttle,useDocumentBottom;return $window=angular.element($window),scrollDistance=null,scrollEnabled=null,checkWhenEnabled=null,container=null,immediateCheck=!0,useDocumentBottom=!1,handler=function(){var containerBottom,containerTopOffset,elementBottom,remaining,shouldScroll;return container===$window?(containerBottom=container.height()+container.scrollTop(),elementBottom=elem.offset().top+elem.height()):(containerBottom=container.height(),containerTopOffset=0,void 0!==container.offset()&&(containerTopOffset=container.offset().top),elementBottom=elem.offset().top-containerTopOffset+elem.height()),useDocumentBottom&&(elementBottom=$(document).height()),remaining=elementBottom-containerBottom,shouldScroll=remaining<=container.height()*scrollDistance+1,shouldScroll?(checkWhenEnabled=!0,scrollEnabled?scope.$$phase||$rootScope.$$phase?scope.infiniteScroll():scope.$apply(scope.infiniteScroll):void 0):checkWhenEnabled=!1},throttle=function(func,wait){var later,previous,timeout;return timeout=null,previous=0,later=function(){var context;return previous=(new Date).getTime(),$timeout.cancel(timeout),timeout=null,func.call(),context=null},function(){var now,remaining;return now=(new Date).getTime(),remaining=wait-(now-previous),0>=remaining?(clearTimeout(timeout),$timeout.cancel(timeout),timeout=null,previous=now,func.call()):timeout?void 0:timeout=$timeout(later,remaining)}},null!=THROTTLE_MILLISECONDS&&(handler=throttle(handler,THROTTLE_MILLISECONDS)),scope.$on("$destroy",function(){return container.off("scroll",handler)}),handleInfiniteScrollDistance=function(v){return scrollDistance=parseInt(v,10)||0},scope.$watch("infiniteScrollDistance",handleInfiniteScrollDistance),handleInfiniteScrollDistance(scope.infiniteScrollDistance),handleInfiniteScrollDisabled=function(v){return scrollEnabled=!v,scrollEnabled&&checkWhenEnabled?(checkWhenEnabled=!1,handler()):void 0},scope.$watch("infiniteScrollDisabled",handleInfiniteScrollDisabled),handleInfiniteScrollDisabled(scope.infiniteScrollDisabled),handleInfiniteScrollUseDocumentBottom=function(v){return useDocumentBottom=v},scope.$watch("infiniteScrollUseDocumentBottom",handleInfiniteScrollUseDocumentBottom),handleInfiniteScrollUseDocumentBottom(scope.infiniteScrollUseDocumentBottom),changeContainer=function(newContainer){return null!=container&&container.off("scroll",handler),container="function"==typeof newContainer.last&&newContainer!==$window?newContainer.last():newContainer,null!=newContainer?container.on("scroll",handler):void 0},changeContainer($window),handleInfiniteScrollContainer=function(newContainer){if(null!=newContainer&&0!==newContainer.length){if(newContainer=angular.element(newContainer),null!=newContainer)return changeContainer(newContainer);throw new Exception("invalid infinite-scroll-container attribute.")}},scope.$watch("infiniteScrollContainer",handleInfiniteScrollContainer),handleInfiniteScrollContainer(scope.infiniteScrollContainer||[]),null!=attrs.infiniteScrollParent&&changeContainer(angular.element(elem.parent())),null!=attrs.infiniteScrollImmediateCheck&&(immediateCheck=scope.$eval(attrs.infiniteScrollImmediateCheck)),$timeout(function(){return immediateCheck?handler():void 0},0)}}}]);
angular.module("mtg.directives",[]).directive("ngShift",["$rootScope",function($rootScope){return function(scope,element){element.bind("keydown keypress",function(event){(37==event.keyCode||39==event.keyCode)&&$rootScope.$broadcast("shift",event)})}}]);
angular.module("mtg.services",[]).factory("API",["$resource",function($resource){return{Cards:$resource("http://api.cmandersen.net/mtg/v1/cards/:id",{id:"@id"}),Planes:$resource("http://api.cmandersen.net/mtg/v1/planes/:id",{id:"@id"})}}]).service("Plane",["API","$rootScope",function(API,$rootScope){this.planes=[],this.get=function(callback){return this.planes.length>0?(console.log("Exists"),callback(this.planes),this.planes):(console.log("Getting..."),API.Planes.query(function(planes){$rootScope.$emit("setPlanes",planes),callback(planes)}))},$rootScope.$on("setPlanes",function(planes){this.planes=planes}.bind(this)),$rootScope.$on("randomPlanes",function(){API.Planes.query({randomize:!0},function(planes){$rootScope.$emit("setPlanes",planes),$rootScope.$broadcast("planesUpdated",planes)})})}]);
angular.module("mtg.controllers.cards",[]).controller("CardListCtrl",["$scope","API","$modal","Analytics","cards",function($scope,API,$modal,Analytics,cards){function reset(){$scope.offset=0,$scope.loaded=!0}$scope.type="Type",$scope.colors="Color",$scope.rarity="Rarity",$scope.loaded=!0,$scope.offset=0,$scope.limit=50,$scope.cards=cards,$scope.filterType=function(des){Analytics.trackEvent("Cards","Filter","Type"),reset(),$scope.type=des,$scope.cards=API.Cards.query({limit:$scope.limit,offset:$scope.offset,title:$scope.search,type:$scope.type,colors:$scope.colors,rarity:$scope.rarity})},$scope.filterColors=function(color){Analytics.trackEvent("Cards","Filter","Color"),reset(),$scope.colors=color,$scope.cards=API.Cards.query({limit:$scope.limit,offset:$scope.offset,title:$scope.search,type:$scope.type,colors:$scope.colors,rarity:$scope.rarity})},$scope.filterRarity=function(rar){Analytics.trackEvent("Cards","Filter","Rarity"),reset(),$scope.rarity=rar,$scope.cards=API.Cards.query({limit:$scope.limit,offset:$scope.offset,title:$scope.search,type:$scope.type,colors:$scope.colors,rarity:$scope.rarity})},$scope.cardSearch=function(){Analytics.trackEvent("Cards","Filter","Search"),reset();var name=$scope.search;$scope.cards=API.Cards.query({limit:$scope.limit,offset:$scope.offset,title:name,type:$scope.type,colors:$scope.colors,rarity:$scope.rarity})},$scope.openModal=function(id){for(var index in $scope.cards){var card=$scope.cards[index];card.id==id&&($scope.card=card,$scope.index=index,Analytics.trackEvent("Cards","Click",$scope.card.title))}$scope.modal()},$scope.modal=function(){$modal.open({templateUrl:"assets/html/cards/show.html",controller:"CardModalCtrl",scope:$scope})},$scope.loadNextBatch=function(){$scope.loaded&&($scope.offset+=$scope.limit,API.Cards.query({limit:$scope.limit,offset:$scope.offset,begins:$scope.search,type:$scope.type,colors:$scope.colors,rarity:$scope.rarity},function(data){data.length<$scope.limit-1&&($scope.loaded=!1);for(var card in data)angular.isObject(data[card])&&"title"in data[card]&&$scope.cards.push(data[card])}),Analytics.trackEvent("Cards","Scroll","Page "+$scope.offset/$scope.limit))}}]).controller("CardModalCtrl",["$scope","$modalInstance",function(){}]).controller("PlaneListCtrl",["$scope","API","$routeParams","$location","$modal","$timeout","Analytics","planes",function($scope,API,$routeParams,$location,$modal,$timeout,Analytics,planes){$scope.planes=planes,$scope.selectRandom=function(){Analytics.trackEvent("Planes","Interact","Randomize"),$scope.planes=API.Planes.query({randomize:1})},$scope.openModal=function(id){for(var index in $scope.planes){var plane=$scope.planes[index];plane.id==id&&($scope.plane=plane,$scope.index=index,Analytics.trackEvent("Planes","Click",$scope.plane.title))}$scope.modal()},$scope.shift=function(event){37==event.keyCode?($scope.index--,$scope.index<0&&($scope.index=$scope.planes.length-1)):39==event.keyCode&&($scope.index++,$scope.index==$scope.planes.length&&($scope.index=0)),Analytics.trackEvent("Planes","Interact","Shift"),$scope.plane=$scope.planes[$scope.index],$scope.modal()},$scope.modal=function(){var modal=$modal.open({templateUrl:"assets/html/cards/planes/show.html",controller:"PlaneModalCtrl",scope:$scope});modal.opened.then(function(){$timeout(function(){jQuery(".modal").keydown(function(e){(37==e.keyCode||39==e.keyCode)&&(modal.close(),$scope.shift(e))})},100)})}}]).controller("PlaneModalCtrl",["$scope","$modalInstance",function($scope,$modalInstance){var removeListen=$scope.$on("shift",function(evt,event){removeListen(),$modalInstance.close(),$scope.shift(event)})}]);
var app=angular.module("mtg",["ngRoute","ngResource","ui.bootstrap","mtg.directives","mtg.services","mtg.controllers.cards","napp.menu","angular-google-analytics","infinite-scroll"]);
app.config(["$routeProvider","AnalyticsProvider",function($routeProvider,AnalyticsProvider){AnalyticsProvider.setAccount("UA-48980840-2"),$routeProvider.when("/",{templateUrl:"assets/html/dashboard.html"}).when("/cards",{templateUrl:"assets/html/cards/list.html",controller:"CardListCtrl",resolve:{cards:["API",function(API){return API.Cards.query()}]}}).when("/cards/:id",{templateUrl:"assets/html/cards/show.html",controller:"CardCtrl"}).when("/planes",{templateUrl:"assets/html/cards/planes/list.html",controller:"PlaneListCtrl",resolve:{planes:["API",function(API){return API.Planes.query()}]}}).when("/planes/:id",{templateUrl:"assets/html/cards/planes/show.html",controller:"PlaneCtrl"}).otherwise({redirectTo:"/"})}]);
app.run(["Analytics",function(){}]);