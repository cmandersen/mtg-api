...
angular.module("mtg.controllers.cards",[]).controller("CardListCtrl",["$scope","API","$modal","cards",function($scope,API,$modal,cards){function reset(){$scope.offset=0,$scope.loaded=!0}$scope.type="Type",$scope.colors="Color",$scope.rarity="Rarity",$scope.loaded=!0,$scope.offset=0,$scope.limit=50,$scope.cards=cards,$scope.filterType=function(des){reset(),$scope.type=des,$scope.cards=API.Cards.query({limit:$scope.limit,offset:$scope.offset,title:$scope.search,type:$scope.type,colors:$scope.colors,rarity:$scope.rarity})},$scope.filterColors=function(color){reset(),$scope.colors=color,$scope.cards=API.Cards.query({limit:$scope.limit,offset:$scope.offset,title:$scope.search,type:$scope.type,colors:$scope.colors,rarity:$scope.rarity})},$scope.filterRarity=function(rar){reset(),$scope.rarity=rar,$scope.cards=API.Cards.query({limit:$scope.limit,offset:$scope.offset,title:$scope.search,type:$scope.type,colors:$scope.colors,rarity:$scope.rarity})},$scope.cardSearch=function(){reset();var name=$scope.search;$scope.cards=API.Cards.query({limit:$scope.limit,offset:$scope.offset,title:name,type:$scope.type,colors:$scope.colors,rarity:$scope.rarity})},$scope.openModal=function(id){for(var index in $scope.cards){var card=$scope.cards[index];card.id==id&&($scope.card=card,$scope.index=index)}$scope.modal()},$scope.modal=function(){$modal.open({templateUrl:"assets/html/cards/show.html",controller:"CardModalCtrl",scope:$scope})},$scope.loadNextBatch=function(){$scope.loaded&&($scope.offset+=$scope.limit,API.Cards.query({limit:$scope.limit,offset:$scope.offset,begins:$scope.search,type:$scope.type,colors:$scope.colors,rarity:$scope.rarity},function(data){data.length<$scope.limit-1&&($scope.loaded=!1);for(var card in data)angular.isObject(data[card])&&"title"in data[card]&&$scope.cards.push(data[card])}))}}]).controller("CardModalCtrl",["$scope","$modalInstance",function(){}]).controller("PlaneListCtrl",["$scope","API","$routeParams","$location","$modal","$timeout","planes",function($scope,API,$routeParams,$location,$modal,$timeout,planes){$scope.planes=planes,$scope.selectRandom=function(){$scope.planes=API.Planes.query({randomize:1})},$scope.openModal=function(id){for(var index in $scope.planes){var plane=$scope.planes[index];plane.id==id&&($scope.plane=plane,$scope.index=index)}$scope.modal()},$scope.shift=function(event){37==event.keyCode?($scope.index--,$scope.index<0&&($scope.index=$scope.planes.length-1)):39==event.keyCode&&($scope.index++,$scope.index==$scope.planes.length&&($scope.index=0)),$scope.plane=$scope.planes[$scope.index],$scope.modal()},$scope.modal=function(){var modal=$modal.open({templateUrl:"assets/html/cards/planes/show.html",controller:"PlaneModalCtrl",scope:$scope});modal.opened.then(function(){$timeout(function(){jQuery(".modal").keydown(function(e){(37==e.keyCode||39==e.keyCode)&&(modal.close(),$scope.shift(e))})},100)})}}]).controller("PlaneModalCtrl",["$scope","$modalInstance",function($scope,$modalInstance){var removeListen=$scope.$on("shift",function(evt,event){removeListen(),$modalInstance.close(),$scope.shift(event)})}]);
